...
namespace godot {
	...
	void PlayerControl::_register_methods() {
		...
		register_property<PlayerControl, Vector3>("puppet_position", &PlayerControl::puppet_position, Vector3(0, 0, 0), GODOT_METHOD_RPC_MODE_SLAVE);
		register_property<PlayerControl, Vector3>("puppet_motion", &PlayerControl::puppet_motion, Vector3(0, 0, 0), GODOT_METHOD_RPC_MODE_SLAVE);
	...
	void PlayerControl::start() {
		player = std::make_shared<KinematicBody>(*static_cast<KinematicBody*>(get_parent()));
	...
	void PlayerControl::_physics_process(float delta) {
		if (is_network_master()) {
            ...
            if(player->call("is_on_floor")) {
                if(motion.y < 0)
                    motion.y = 0;
                if (input->is_action_just_pressed("pl_jump") && !busy) {
                    player->call("jump");
                }
            }
			...
			rset_unreliable("puppet_position", player->get_translation());
		} else {
			player->set_translation(puppet_position);
		...
		player->call("move_and_slide", normal_motion * player_speed,  -gravity.normalized());
		player->call("start_animation", abs(normal_motion.x)+abs(normal_motion.z));

		if (!is_network_master()) {
			puppet_position = player->get_translation();
		}
